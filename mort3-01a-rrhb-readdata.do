...
//MODIFIED BY HAMA TO INCLUDE CIRCULATORY SUBCATEGOIRES//

*-----------------------------------------------------------------------------------------
* READ DATA AND APPLY OFFSETS WITH THE ADDITION OF CIRCULATORY SUBCATEGORIES 
*-----------------------------------------------------------------------------------------
...
//read circulatory subcategories (HBRR classification)
merge 1:1 lngpk_bccss o_indexno using "$data/BCCSSMortHBRRCat.dta" , ///
keepusing(circ_cat_hbrr)
//check what the 29 unmatched cases look like
list circ_cat_hbrr if _merge==2
//proceed once confirmed
assert _merge==3 | _merge==2
keep if _merge==3
drop _merge

//Recode the circulatory subcategories into 3 broader groups
gen circsubcat = .
replace circsubcat = 1 if inrange(circ_cat_hbrr, 1, 5)   // all cardiac
replace circsubcat = 2 if circ_cat_hbrr == 6              // cerebrovascular
replace circsubcat = 3 if circ_cat_hbrr == 7               // other circulatory
label define lblcircsubcat 1 "cardiac" 2 "cerebrovascular" 3 "other circulatory"
label values circsubcat lblcircsubcat
label var circsubcat "circulatory subcategory (HBRR, grouped)"

tab circ_cat_hbrr circsubcat, miss
tab circsubcat, miss
